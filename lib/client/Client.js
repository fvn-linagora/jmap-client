...
export default class Client {
   ...
   *
   * @param filteringRules {FilterRule[]} The {@link FilterRule} instance to set
   ...
   * @returns {Promise} A {@link Promise} that eventually resolves to nothing upon success.
   */
  setFilter(filteringRules, options) {
    Utils.assertRequiredParameterHasType(filteringRules, 'filteringRules', InboxFilter);

    return this._jmapRequest('setFilter', {
      accountId: options && options.accountId,
      ifInState: options.ifInState,
      update: {
        [FilterRule.ID]: filteringRules.toJSONObject()
      }
    }).then(response => {
