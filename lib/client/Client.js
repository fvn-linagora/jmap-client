...
export default class Client {
  ...
  _filterKeywords(message) {
    var keywords = message.keywords || {};
    message.keywords = Object.keys(keywords)
      .filter( key => keywords[key] )
      .reduce( (res, key) => Object.assign(res, { [key]: keywords[key] }), {} )
    return message;
  }
  ...
  _createMessage(message, role, mailbox) {
    ...
    let clientId = this._generateClientId(),
        doSetMessages = mailbox => {
          ...
          message.keywords.$Draft = MailboxRole.DRAFTS.value === role.value;
          message = this._filterKeywords(message);
          // const {ignored, ..._keywords} = this.keywords;
          // { keywords: {..._keywords, $Flagged: isFlagged } }
